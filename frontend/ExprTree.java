...
public class ExprTree extends Tree {
	...
	public Type cast_type;	// also type for instanceof. frontend.Parser fills this in for new operator as well
	...
	public void resolveNames (Context ctx, boolean dotchain) throws CompilerException {
		...
		if (op != null && op.type == Toktype.DOT) {
			...
			if (t instanceof Klass) {
				...
				Log.write ("type.Array attribute reference not yet supported");
		...
		switch (type) {
			case ID:
				...
				} else { 	//if (parent.type == frontend.Treetype.OP && ((frontend.ExprTree) parent).op.type == frontend.Toktype.DOT) {
			...
			case AGG:
				/*
				frontend.ExprTree e = left;
				if (! (dtype instanceof type.Array)) common.Log.fatal( new common.InternalError ("Aggregate has non-array type?!"));
				type.Array arr = (type.Array) dtype;
				while (e != null) {
					if (e.dtype != arr.basetype) common.Log.error( new SemanticException ("Aggregate element has wrong type (" + e.dtype + "); should be " + arr.basetype));
					e = (frontend.ExprTree) e.next;
				}
				*/
				break;
			case FUNCALL:
	...
	public Type resolveOperatorType () throws CompilerException {
		...
		/* type.Type restrictions on operators:
		 *
		...
		*/
		switch (op.type) {
			...
			case LBRACK:
				...
				if (!right.dtype.isIntegral()) Log.error( new SemanticException ("type.Array index must be an integral type"));
		...
		Log.fatal( new InternalError("Unimplemented operator frontend.Toktype in resolveOperatorType()"));
		return null;
	}
