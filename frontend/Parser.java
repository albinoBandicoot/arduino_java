...
public class Parser {
	...
	private void checkIllegalName(String s) throws CompilerException {
		if (s.equals("this") || s.equals("super"))
			Log.error(new ParserException("'" + s + "' is a reserved word and cannot be used as an identifier"));
	}
			...
			while (cont) {
				...
				if (on(Toktype.SEMICOLON) || on(Toktype.ASSIGN)) {
					...
					if (t.ext == Type.Ext.ABSTRACT) {
						if (!ona(Toktype.SEMICOLON))
							Log.error(new ParserException("Expected semicolon after abstract function declaration"));
					} else if (t.isNative) {
						if (!ona(Toktype.SEMICOLON))
							Log.error(new ParserException("Expected semicolon after native function declaration"));
					} else {
						t.body = block();
					}
					list.add(t);
	...
	public Tree stmt() throws CompilerException {
		...
		ExprTree t = expr();
		if (!t.isValidStatement()) Log.error(new ParserException("Not a valid statement"));
		if (!ona(Toktype.SEMICOLON)) Log.error(new ParserException("Expecting semicolon after statement"));
		return t;
	...
	public Tree for_stmt() throws CompilerException {
		...
		if (t.ch.isEmpty()) {    // wasn't a varinit. get an expr
			t.ch.add(expr());
			if (!ona(Toktype.SEMICOLON))
				Log.error(new ParserException("Expecting semicolon after for loop initializer"));
		}
	...
	public Tree delete_stmt() throws CompilerException {
		...
		if (!ona(Toktype.SEMICOLON))
			Log.error(new ParserException("Expected semicolon after list of objects to delete"));
		return t;
	}
	...
	public ExprTree En(int i) throws CompilerException {
		switch (i) {
			case -2:
				...
				return k;
			case -1:
				return E(i, Toktype.DBLOR);
			case 0:
				return E(i, Toktype.DBLAND);
			case 1:
				return E(i, Toktype.OR);
			case 2:
				return E(i, Toktype.XOR);
			case 3:
				return E(i, Toktype.AND);
			case 4:
				return E(i, Toktype.EQ, Toktype.NE);
			case 5:
				return E(i, Toktype.LE, Toktype.LT, Toktype.GT, Toktype.GE);
			case 6:        // instanceof
				ExprTree q = En(7);
				...
				return q;
			case 7:
				return E(i, Toktype.LSHIFT, Toktype.RSHIFT, Toktype.ULSHIFT);
			case 8:
				return E(i, Toktype.PLUS, Toktype.MINUS);
			case 9:
				return E(i, Toktype.TIMES, Toktype.DIVIDE, Toktype.MOD);
			case 10:
				if (on(Toktype.INC) || on(Toktype.DEC)) {    // preincrement
					...
					ExprTree e = En(11);
					if (e.type == Treetype.TYPE) {
						e.type = Treetype.CAST;
						e.cast_type = Primitive.parse(e.op.lexeme);
						e.left = expr();
					} else if (e.type == Treetype.ID && Compiler.classExists(e.op.lexeme)) {
						e.type = Treetype.CAST;
						e.cast_type = new Klass(e.op.lexeme);
						e.left = expr();
					} else {
						// not a cast.
					}
	...
	public ExprTree allocator() throws CompilerException {
	...
	public ExprTree dotchain() throws CompilerException {
		if (on(Toktype.PRIMTYPE)) {
			ExprTree e = new ExprTree(Treetype.TYPE, tok);
			advance();
			return e;
		}
		ExprTree e = lvalue_first_comp();
		// ???		return null;	// to allow empty expressions
		while (true) {
			if (on(Toktype.DOT)) {
	...
	public ExprTree lvalue_first_comp() throws CompilerException {
		if (ona(Toktype.LPAREN)) {
			ExprTree e = expr();
	...
	public ExprTree lvalue_comp() throws CompilerException {
		if (ona(Toktype.NEW)) return allocator();
		if (!on(Toktype.ID)) Log.fatal(new ParserException("Expected ID after '.'"));
		ExprTree e = new ExprTree(Treetype.ID, tok);
		e.name = tok.lexeme;
		...
		if (ona(Toktype.LBRACE)) {
			...
			while (!ona(Toktype.RBRACE)) {
				if (on(Toktype.LBRACE)) {    // nested aggregate
					if (!first && !nested)
						Log.error(new ParserException("Aggregate contains a mixture of scalar and aggregate entries"));
					nested = true;
					c.add(aggregate());
				} else {
					if (!first && nested)
	...
	public PragmaSet pragmas () throws CompilerException {
		PragmaSet ps = new PragmaSet();
		while (ona(Toktype.PRAGMA)) {
			ps.add(pragma());
		}
		return ps;
	}

	public Pragma pragma () throws CompilerException {
		if (on(Toktype.ID)) {
			Pragma p = new Pragma(tok);
			advance();
			if (ona(Toktype.LPAREN)) {
				do {
					p.args.add(tok.lexeme);
					advance();
				} while (ona(Toktype.COMMA));
				if (!ona(Toktype.RPAREN)) Log.error(new ParserException("Expected ) to close pragma arguments"));
			}
			return p;
		} else {
			Log.fatal (new ParserException("Expecting ID for pragma type identifier"));
		}
		return null;
	}
}
